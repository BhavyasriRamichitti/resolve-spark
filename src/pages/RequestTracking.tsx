                ...
                {filteredRequests.map((request, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-medium text-black text-sm">{request.title}</h3>
                      <Badge className={`text-xs text-black ${getStatusColor(request.status)}`}>
                        {request.status.replace('_', ' ')}
                      </Badge>
                    </div>
                    <p className="text-black/70 text-xs mb-3 line-clamp-2">{request.description}</p>
                    <div className="flex items-center justify-between">
                      <Badge className={`text-xs text-black ${getPriorityColor(request.priority)}`}>
                        {request.priority}
                      </Badge>
                      <span className="text-black/50 text-xs">#{request.id}</span>
                    </div>
                  </Card>
                ))}
              </div>
              ...
              <Card className="glass-card p-6">
                <div className="mb-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h2 className="text-2xl font-semibold text-black mb-2">{selectedRequest.title}</h2>
                      <p className="text-black/70">Request #{selectedRequest.id}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className={`text-black ${getPriorityColor(selectedRequest.priority)}`}>
                        {selectedRequest.priority}
                      </Badge>
                      <Badge className={`text-black ${getStatusColor(selectedRequest.status)}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white/5 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-black/70 text-sm">Requester</span>
                      </div>
                      <p className="text-black font-medium">{selectedRequest.requester}</p>
                    </div>
                    <div className="bg-white/5 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <User className="w-4 h-4 text-black/70" />
                        <span className="text-black/70 text-sm">Assigned To</span>
                      </div>
                      <p className="text-black font-medium">{selectedRequest.assignedTo}</p>
                    </div>
                    <div className="bg-white/5 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <Clock className="w-4 h-4 text-black/70" />
                        <span className="text-black/70 text-sm">Due Date</span>
                      </div>
                ...
                <Tabs defaultValue="timeline" className="w-full">
                  <TabsList className="grid w-full grid-cols-3 bg-white/10">
                    <TabsTrigger value="timeline" className="text-black data-[state=active]:bg-primary">Timeline</TabsTrigger>
                    <TabsTrigger value="details" className="text-black data-[state=active]:bg-primary">Details</TabsTrigger>
                    <TabsTrigger value="attachments" className="text-black data-[state=active]:bg-primary">Files</TabsTrigger>
                  </TabsList>
                        ...
                        {getTimelineSteps(selectedRequest.status).map((step, index) => (
                          <div key={index} className="relative flex items-center mb-8">
                            <div className={`absolute left-0 w-8 h-8 rounded-full border-2 flex items-center justify-center ${
                              ...
                              {step.completed && <CheckCircle2 className="w-4 h-4 text-black" />}
                            </div>
                            <div className="ml-12">
                              <p className={`font-medium ${step.completed ? 'text-black' : 'text-black/50'}`}>
                                {step.label}
                              </p>
                              {step.timestamp && (
                                <p className="text-black/50 text-sm">{step.timestamp}</p>
                              )}
                            </div>
                          </div>
                        ))}
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <span className="text-black/50 text-sm">{comment.timestamp}</span>
                                </div>
                                <p className="text-black/80">{comment.message}</p>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  <TabsContent value="details" className="mt-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-black/80 bg-white/5 rounded-lg p-4">{selectedRequest.description}</p>
                      </div>
                      <div>
                        <h3 className="text-black font-medium mb-2">Category</h3>
                        <p className="text-black/80">{selectedRequest.category}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <h3 className="text-black font-medium mb-2">Created</h3>
                          <p className="text-black/80">{new Date(selectedRequest.createdAt).toLocaleString()}</p>
                        </div>
                        <div>
                          <h3 className="text-black font-medium mb-2">Last Updated</h3>
                          <p className="text-black/80">{new Date(selectedRequest.updatedAt).toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="attachments" className="mt-6">
                    <div className="text-center py-8">
                      <Paperclip className="w-12 h-12 text-black/30 mx-auto mb-4" />
                      <p className="text-black/70">No attachments for this request</p>
                    </div>
                  </TabsContent>
                </Tabs>
              </Card>
